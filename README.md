...
[![DOI](https://zenodo.org/badge/DOI/10.5281/zenodo.18116411.svg)](https://doi.org/10.5281/zenodo.18116411)
...
[Citation](#citation) · [Dataset](#dataset) · [Benchmark Results](#benchmark-results) · [Quickstart](#quickstart) · [Model Zoo](#model-zoo)
...
<img src="Assets/A_fwd_scalar_r2_heatmap_dark.png" width="100%" alt="Forward Scalar R2 Heatmap"/>
<img src="Assets/B_fwd_scalar_hexbin_dark.png" width="100%" alt="Forward Scalar Hexbin"/>
<img src="Assets/D_fwd_scalar_radar_dark.png" width="100%" alt="Forward Scalar Radar"/>
<img src="Assets/E2_fwd_scalar_curves_dark.png" width="100%" alt="Forward Scalar Curves"/>
<img src="Assets/F_spec_gallery_dark.png" width="100%" alt="Spectrum Gallery"/>

We benchmark models across four complementary tasks spanning the full forward–inverse design loop on 500,000 simulated grating coupler geometries from the **GC-500K** dataset.
...
The **GC-500K** dataset is a large-scale, physics-consistent synthetic dataset for photonic grating coupler analysis and data-driven modeling. It is publicly available on Zenodo.
...
| Simulation method | Validated analytical model (physics-consistent) |
| Energy conservation | Enforced via A = 1 − R − T closure |

### Download

Zenodo (primary): https://doi.org/10.5281/zenodo.18116411

```bash
wget https://zenodo.org/record/18116411/files/gc_500k_.h5
```

Or download directly from the Zenodo page above.

### Geometry Parameters

| Parameter | Symbol | Range |
|---|---|---|
| Grating period | `period_nm` | 300–700 nm |
| Fill factor | `fill_factor` | 0.3–0.7 |
| Etch depth | `etch_depth_nm` | 50–200 nm |
| Oxide thickness | `oxide_thickness_nm` | 1000–2000 nm |
| Silicon thickness | `si_thickness_nm` | 200–300 nm |
...
```bash
git clone https://github.com/Reckonchamp12/GC_500K_BENCHMARK.git
cd GC_500K_BENCHMARK
pip install -e .
```
...
Download the GC-500K dataset from Zenodo:

```bash
wget https://zenodo.org/record/18116411/files/gc_500k_.h5
```

Or visit: https://doi.org/10.5281/zenodo.18116411

Set the path in `configs/default.yaml`:

```yaml
data_path: /path/to/gc_500k_.h5
```
...
cfg = CFG(data_path="/path/to/gc_500k_.h5")
data = load_data(cfg)
...
If you use GC-Bench or the GC-500K dataset in your research, please cite:

### Dataset

```bibtex
@dataset{ray2026gc500k,
  author    = {Ray, Rahul D.},
  title     = {GC-500K: A Physics-Consistent Synthetic Dataset for Photonic Grating Coupler Design},
  year      = {2026},
  publisher = {Zenodo},
  doi       = {10.5281/zenodo.18116411},
  url       = {https://doi.org/10.5281/zenodo.18116411}
}
```
...
@misc{gcbench2026,
  ...
  author    = {Ray, Rahul D. and Yeleswarapu, Ravikiran and Gera, Anuvansh},
  year      = {2026},
...
The GC-500K dataset is hosted on Zenodo and licensed under **CC-BY-4.0**.
